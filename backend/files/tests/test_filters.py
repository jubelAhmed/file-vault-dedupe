...
class FileFilterTestCase(APITestCase):
    ...
    def test_file_list_filter_by_type(self):
        ...
        response = self.client.get(
            '/api/files/?file_type=text/plain', 
            HTTP_UserId=self.user_id
        )
    ...
    def test_file_list_filter_by_date_end(self):
        ...
        if len(files) < 3:
            # seed if needed
            self.client.post('/api/files/', {'file': SimpleUploadedFile('a.txt', b'A', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            self.client.post('/api/files/', {'file': SimpleUploadedFile('b.txt', b'B', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            self.client.post('/api/files/', {'file': SimpleUploadedFile('c.txt', b'C', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            files = list(File.objects.filter(user_id=self.user_id).order_by('uploaded_at'))
    ...
    def test_file_list_filter_by_date_start_end(self):
        ...
        if len(files) < 3:
            # seed if needed
            self.client.post('/api/files/', {'file': SimpleUploadedFile('x.txt', b'X', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            self.client.post('/api/files/', {'file': SimpleUploadedFile('y.txt', b'YY', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            self.client.post('/api/files/', {'file': SimpleUploadedFile('z.txt', b'ZZZ', content_type='text/plain')}, format='multipart', HTTP_UserId=self.user_id)
            files = list(File.objects.filter(user_id=self.user_id).order_by('uploaded_at'))
    ...
    def test_file_list_filter_combination_all(self):
        ...
        self.client.post('/api/files/', {'file': f_txt_small}, format='multipart', HTTP_UserId=self.user_id)
        self.client.post('/api/files/', {'file': f_txt_large}, format='multipart', HTTP_UserId=self.user_id)
        self.client.post('/api/files/', {'file': f_other}, format='multipart', HTTP_UserId=self.user_id)

        # Apply filters to match only report_big.txt (200 bytes)
        query = (
            '/api/files/?search=big&file_type=text/plain'
            '&min_size=150&max_size=250'
        )
    ...
    def test_file_list_filter_date_only_ignores_time(self):
        ...
        files[0].uploaded_at = timezone.make_aware(
            timezone.datetime.combine(target_date, timezone.datetime.min.time().replace(hour=9))  # 9 AM
        )
        files[0].save(update_fields=['uploaded_at'])
        
        files[1].uploaded_at = timezone.make_aware(
            timezone.datetime.combine(target_date, timezone.datetime.min.time().replace(hour=14))  # 2 PM
        )
        files[1].save(update_fields=['uploaded_at'])
        
        files[2].uploaded_at = timezone.make_aware(
            timezone.datetime.combine(target_date, timezone.datetime.min.time().replace(hour=20))  # 8 PM
        )
        files[2].save(update_fields=['uploaded_at'])
        
        # Filter by the target date - should get all 3 files regardless of time
        date_str = target_date.strftime('%Y-%m-%d')
        response = self.client.get(f'/api/files/?start={date_str}&end={date_str}', HTTP_UserId=self.user_id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
