...
class SearchIndexingService:
    ...
    def extract_keywords(text: str) -> Set[str]:
        ...
        stop_words = getattr(settings, 'SEARCH_INDEX_STOP_WORDS', set())
    ...
    def index_file_content(file_instance: File, text_content: str) -> int:
        ...
        for keyword in keywords:
            try:
                ...
                search_index, created = FileSearchIndex.objects.get_or_create(
                    keyword=keyword
                )
                ...
                logger.error(f"Error indexing keyword '{keyword}' for file {file_instance.id}: {str(e)}")
                continue
    ...
    @staticmethod
    def search_files_by_keyword(keyword: str, user_id: Optional[str] = None) -> List[File]:
        """
    ...
    @staticmethod
    def search_files_by_keywords(keywords: List[str], user_id: Optional[str] = None) -> List[File]:
        """
        ...
        try:
            ...
            search_indexes = FileSearchIndex.objects.filter(
                keyword__in=normalized_keywords
            )
            ...
            logger.info(f"Found {files.count()} files for keywords: {', '.join(normalized_keywords)}")
            return list(files)
    ...
    def get_keyword_stats() -> dict:
        ...
        try:
            ...
            top_keyword = FileSearchIndex.objects.annotate(
            ...
            return {
                'total_keywords': 0,
                'top_keyword': {'keyword': None, 'file_count': 0}
            }
