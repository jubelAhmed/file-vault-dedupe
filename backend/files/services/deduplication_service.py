...
class DeduplicationService:
    ...
    def handle_file_upload(cls, user_id, uploaded_file):
        ...
        if existing_file:
            ...
            new_file = cls._create_file_reference(
                user_id, uploaded_file, file_hash, existing_file
            )
            # Update only original storage (no actual storage used)
            StorageService.update_storage(
                user_id, uploaded_file.size, original_only=True
            )
        else:
            # Create new original file
            new_file = cls._create_original_file(
                user_id, uploaded_file, file_hash
            )
            # Update both storage counters
            StorageService.update_storage(
                user_id, uploaded_file.size, original_only=False
            )
    ...
    def handle_file_deletion(cls, file_obj):
        ...
        if file_obj.is_reference:
            ...
            StorageService.update_storage(
                file_obj.user_id, file_obj.size, 
                original_only=True, subtract=True
            )
            file_obj.delete()
            return True
        else:
            ...
            if ref_count == 0:
                ...
                StorageService.update_storage(
                    file_obj.user_id, file_obj.size,
                    original_only=False, subtract=True
                )
                file_obj.delete()
                return True
            else:
    ...
    def get_deduplication_stats(cls):
        ...
        total_original_storage = (
            File.objects.aggregate(total=Sum('size'))['total'] or 0
        )
        ...
        savings_percentage = (
            (storage_savings / total_original_storage * 100)
            if total_original_storage > 0 else 0
        )
